...
import CitationUpload from "@/components/intake/CitationUpload";
import CitationReview from "@/components/intake/CitationReview";
import ContactInfo from "@/components/intake/ContactInfo";
...
function FightMyTicketContent() {
  ...
  useEffect(() => {
    ...
    if (success === "true" && sessionId) {
      ...
      const saved = localStorage.getItem("intakeState");
      if (saved) {
        try {
      ...
      localStorage.removeItem("intakeState");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        {state.step === "confirmation" && (
          <Confirmation state={state} onReset={resetForm} />
        )}
      </div>
    </div>
  );
...
export default function FightMyTicketPage() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <FightMyTicketContent />
    </Suspense>
  );
}
