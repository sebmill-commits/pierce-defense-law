...
import { SEATTLE_COURT_PRICING, calculateSeattlePrice } from "@/lib/seattle-constants";
...
const courts = Object.keys(SEATTLE_COURT_PRICING).filter((c) => c !== "default");
...
export default function SeattleCitationReview({
  citation,
  updateCitation,
  price,
  setPrice,
  ...
  useEffect(() => {
    if (citation.courtName) {
      const priceInfo = calculateSeattlePrice(citation.courtName, citation.violationType);
